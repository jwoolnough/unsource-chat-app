...
const Message = ({
  ...
  return (
    <div className="grid grid-cols-[1.5rem_1fr] items-end gap-4 sm:grid-cols-[2rem_1fr]">
      {showAvatar && !isMe && (
        <div className="h-6 w-6 rounded-full bg-slate-300 sm:h-8 sm:w-8"></div>
      )}

      <div className={clsxm("col-start-2", isMe && "text-right")}>
        <div className="inline-block max-w-[calc(100%-2.5rem)] text-left sm:max-w-[80%]">
          {showDate && (
            <p className="mb-2 mt-[1.125rem] text-[0.625rem] text-slate-400">
              Jordan, {formatDistanceToNow(parseISO(createdAt))} ago
            </p>
          )}
          <div
            className={clsxm(
